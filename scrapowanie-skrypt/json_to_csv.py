...
def generate_csv_files(root_path):
        ...
        for filename in filenames:
            if filename.endswith('.json'):
                ...
                for item in data:
                    ...
                    product = item['name']
                    price = float(item['price']) if item['price'] else 0.0  # Convert price to float, handle empty string
                    brand = item['brand']
    ...
    csv_files = [('../scrapowanie-wynik/CSV/departments.csv', departments), ('../scrapowanie-wynik/CSV/categories.csv', categories), 
                 ('../scrapowanie-wynik/CSV/subcategories.csv', subcategories), ('../scrapowanie-wynik/CSV/products.csv', products)]
