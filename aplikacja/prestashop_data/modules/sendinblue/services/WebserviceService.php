    ...
    public function deleteSendinblueWebserviceKey()
    {
        ...
        try {
            $this->dbInstance->delete(
                ...
                sprintf('id_webservice_account = %s', (int) $accountId)
            );

            $this->dbInstance->delete(
                'webservice_account_shop',
                sprintf('id_webservice_account = %s AND id_shop = %s', (int) $accountId, (int) $this->shopId)
            );
            ...
            foreach ($keys as $key) {
                if ($key['key'] == $webserviceKey) {
                    $this->dbInstance->delete(
                        'webservice_account',
                        sprintf('id_webservice_account = %s', (int) $key['id_webservice_account'])
                    );
