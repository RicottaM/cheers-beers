...
class CustomerSyncCommand extends BaseApiCommand
{
     ...
     */
    public function __construct(Context $context, MailChimp $mailchimp, $customerIds = [], $idStore = null)
    {
            ...
            if ($this->method === self::SYNC_METHOD_POST || $this->method === self::SYNC_METHOD_PUT || $this->method === self::SYNC_METHOD_PATCH) {
                if ($this->syncMode == self::SYNC_MODE_BATCH) {
                    ...
                    $this->commands[$customerId]['members'] = [
                        'route' => "/lists/{$listId}/members/{$hash}",
                        'data' => $listMemberFormatter->format(),
                    ];
                    if (in_array(\Configuration::get(\MailchimpProConfig::CUSTOMER_SYNC_TAG_DEFAULT_GROUP), ["all", "default"]) || \Configuration::get(\MailchimpProConfig::CUSTOMER_SYNC_TAG_GENDER)) {
                        $tags = [];
    ...
    public function getCustomerGenderName($customerId) {
        ...
        $dbquery->where('c.`id_customer` = ' . $customerId);
